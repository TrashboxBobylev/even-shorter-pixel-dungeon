...
import com.shatteredpixel.shatteredpixeldungeon.actors.buffs.Cooldown;
...
public enum Talent {
	...
	//Rogue T2
	MYSTICAL_MEAL(68), INSCRIBED_STEALTH(69), WIDE_SEARCH(70), SILENT_STEPS(71), TRAPPER_MASTERY(72),
	//Rogue T3
	...
	//Duelist T2
	FOCUSED_MEAL(132), LIQUID_AGILITY(133), WEAPON_RECHARGING(134), LETHAL_HASTE(135), SWIFT_EQUIP(136), DESPERATE_CHARGE(136),
	//Duelist T3
	PRECISE_ASSAULT(137, 3), DEADLY_FOLLOWUP(138),
	//Champion T3
	...
	public static void onPotionUsed( Hero hero, int cell, float factor ){
		...
		if (hero.hasTalent(LIQUID_NATURE)){
			...
			for (int grassCell : grassCells){
				...
				if (ch != null && ch.alignment == Char.Alignment.ENEMY){
					//1/2 turns of roots
					Buff.affect(ch, Roots.class, factor * (2 + hero.pointsInTalent(LIQUID_NATURE)));
				}
			...
			// 4/6 cells total
			int totalGrassCells = (int) (factor * (3 + 3 * hero.pointsInTalent(LIQUID_NATURE)));
		...
		if (hero.hasTalent(LIQUID_AGILITY)){
			Buff.prolong(hero, LiquidAgilEVATracker.class, hero.cooldown() + Math.max(0, factor));
			if (factor >= 0.5f){
				Buff.prolong(hero, LiquidAgilACCTracker.class, 5f).uses = Math.round(factor+1);
			}
		}
	}
	...
	public static int onAttackProc( Hero hero, Char enemy, int dmg ){
		...
		if (hero.hasTalent(PATIENT_STRIKE)){
			if (hero.buff(PatientStrikeTracker.class) != null
				...
				dmg += Random.IntRange(1 + hero.pointsInTalent(Talent.PATIENT_STRIKE), 2 + hero.pointsInTalent(PATIENT_STRIKE));
			}
		}
	...
	public static void initClassTalents( HeroClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents, LinkedHashMap<Talent, Talent> replacements ){
		...
		switch (cls){
			...
			case MAGE:
				...
				break;
			case ROGUE:
				Collections.addAll(tierTalents, MYSTICAL_MEAL, INSCRIBED_STEALTH, WIDE_SEARCH, SILENT_STEPS, TRAPPER_MASTERY);
				break;
			case HUNTRESS:
				Collections.addAll(tierTalents, INVIGORATING_MEAL, LIQUID_NATURE, REJUVENATING_STEPS, HEIGHTENED_SENSES, DURABLE_PROJECTILES);
				break;
			case DUELIST:
				Collections.addAll(tierTalents, FOCUSED_MEAL, LIQUID_AGILITY, DEADLY_FOLLOWUP, LETHAL_HASTE, DESPERATE_CHARGE);
				break;
			case CLERIC:
